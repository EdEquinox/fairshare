...
public class InviteController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Fetch the groups and pending invites
        fetchGroups();
        fetchPendingInvites();
    }
    ...
    private void handleBroadcast(ServerResponse response) {
        if (response == null || !response.isSuccess()) {
            Logger.error("Received invalid or unsuccessful broadcast: " + (response != null ? response.message() : "null"));
            return;
        }
        ...
        javafx.application.Platform.runLater(() -> {
            switch (response.message()) {
                case "New invite received":
                    // Refresh the list of pending invites when a new invite is received
                    fetchPendingInvites();
                    AlertUtils.showSuccess("New Invite", "You have received a new invite.");
                    break;
                case "Invite accepted":
                    // Refresh the list of pending invites when an invite is accepted
                    fetchPendingInvites();
                    AlertUtils.showSuccess("Invite Accepted", "An invite has been accepted.");
                    break;
                case "Invite declined":
                    // Refresh the list of pending invites when an invite is declined
                    fetchPendingInvites();
                    AlertUtils.showSuccess("Invite Declined", "An invite has been declined.");
                    break;
                default:
                    Logger.info("Unhandled broadcast message: " + response.message());
            }
        });
    }
        ...
        new Thread(() -> {
            ...
            javafx.application.Platform.runLater(() -> {
                if (response.isSuccess()) {
                    ...
                    inviteList.getItems().clear();  // Clear the existing list
    ...
    public void handleSendInvite() {
        ...
        new Thread(() -> {
            Invite invite = new Invite(
                    0, // ID will be generated by the server
                    selectedGroup.getId(),
                    clientService.getCurrentUser().getId(),
                    0, // ReceiverId will be resolved by the server
                    selectedGroup.getName(),
                    clientService.getCurrentUser().getEmail(),
                    inviteeEmail,
                    Invite.Status.PENDING // Set initial status as PENDING
            );
